...
public class CASTVisitorTrigger extends ASTVisitor {
	...
	private void threadHandle(TypeDeclaration node,ThreadType threadType,String name) {
		...
		String key;
		if (node.resolveBinding().getBinaryName()==null) {
			key = node.resolveBinding().getKey().toString();
		}
		else {
			key = node.resolveBinding().getBinaryName();
		}
	...
	private void acquireTriggerInfo(ASTNode astNode,MethodInvocation node) {
		...
		if(astNode instanceof ClassInstanceCreation){  
			...
			if (isThreadRelate(classInstanceCreation.getType().resolveBinding())) { 				///确保调用对象线程相关
				...
				//(4)变量名(匿名类用key作为变量名)
				String varName;
				if (classInstanceCreation.resolveTypeBinding().getBinaryName()==null) {
					 varName = classInstanceCreation.resolveTypeBinding().getKey();     //binaryName 改 key
				}
				else{
					varName = classInstanceCreation.resolveTypeBinding().getBinaryName();
				}
		...
		if(isThreadRelate(CASTHelper.getInstance().getResolveTypeBinding(astNode))) {      //确保调用对象线程相关	
			...
			//函数返回值等，直接考虑得到的类型
				...
				if (iTypeBinding.getBinaryName()==null) {
					name = iTypeBinding.getKey();     //binaryName 改 key
				}
				else{
					name = iTypeBinding.getBinaryName();
				}
				ThreadTriggerNode threadTiggerNode = new ThreadTriggerNode(CASTHelper.getInstance().getMethodName(node),filePath, lineNumber,name );  
				threadTriggerNodes.add(threadTiggerNode);
			}
		}
	}
	...
	public boolean visit(TypeDeclaration node) {
		...
		if (threadType!=null) {
			...
			if (node.getName().resolveTypeBinding()!=null) {
				if (node.getName().resolveTypeBinding().getBinaryName()==null) {
	...
	public boolean visit(AnonymousClassDeclaration node) {
		...
		if (threadType!=null) {
			...
			if (typeBinding.getBinaryName()==null) {
				anonyName = typeBinding.getKey();   //binaryName 改 key
			}
			else{
				anonyName = typeBinding.getBinaryName();
			}
			//(3)线程信息处理
	...
	public String getBindingTypeName(ClassInstanceCreation node) {
		...
		if (node.resolveTypeBinding()!=null) {
			if (node.resolveTypeBinding().getBinaryName()==null) {
	...
	public boolean visit(ClassInstanceCreation node) {
		...
		//3.匿名创建运行new Thread(){}.start()
			...
			if (isThreadRelate(iTypeBinding)) {	
				...
				String varName;
				if (node.resolveTypeBinding().getBinaryName()==null) {
					varName = node.resolveTypeBinding().getKey();
				}
				else{
					varName = node.resolveTypeBinding().getBinaryName();
				}
				//(4)获取绑定的线程名
				String bindingTypeName = getBindingTypeName(node);  
				if (bindingTypeName==null) {
			...
			for(astNode  = node.getParent();astNode!=compilationUnit;astNode = astNode.getParent()){
				if (astNode instanceof TypeDeclaration) {
					...
					if (typeDeclaration.resolveBinding().getBinaryName()==null) {
						key = typeDeclaration.resolveBinding().getKey()+"_MAIN";
					}
					else{
						key = typeDeclaration.resolveBinding().getBinaryName()+"_MAIN";
					}
					/*
					 * 线程信息：(1)名字&(2)类型
					 */
